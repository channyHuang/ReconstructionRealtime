...
struct Camera_Lidar_queue
{
    ...
    std::vector<std::pair<std::vector<sensor_msgs::Imu::ConstPtr>, sensor_msgs::PointCloudConstPtr>> *m_camera_frame_buf = nullptr;
    ...
    bool if_lidar_can_process()
    {
        ...
        double cam_last_time = get_camera_front_time();
        double lidar_last_time = get_lidar_front_time();
        //printf("if_lidar_can_process cam = %lf, lidar = %lf\n", cam_last_time, lidar_last_time);
