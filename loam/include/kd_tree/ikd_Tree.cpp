...
void KD_TREE::start_thread(){
    ...
    rebuild_thread = std::make_shared< std::thread >(multi_thread_ptr, this);
...
void KD_TREE::stop_thread(){
    ...
    if (rebuild_thread) rebuild_thread->join();
        ...
        if (Rebuild_Ptr != nullptr ){                    
            ...
            if (int(Rebuild_PCL_Storage.size()) > 0){
                ...
                while (!Rebuild_Logger.empty()){
                    ...
                    tmp_counter ++;
                    if (tmp_counter % 10 == 0) std::this_thread::sleep_for(1ms);//usleep(1);
                    //pthread_mutex_lock(&working_flag_mutex);
                    working_flag_mutex.lock();
                    //pthread_mutex_lock(&rebuild_logger_mutex_lock);
                    rebuild_logger_mutex_lock.lock();
