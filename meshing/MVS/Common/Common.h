...
#include <cstring>
//namespace SEACAVE {
extern std::string  g_strWorkingFolder; // empty by default (current folder)
extern std::string  g_strWorkingFolderFull; // full path to current folder
//}
...
#define MAKE_PATH_SAFE(str)	(SEACAVE::Util::isFullPath(str) ? std::string (str) : MAKE_PATH(str)) // add working directory to the given file name only if not full path already
#define MAKE_PATH_FULL(p,s) (SEACAVE::Util::isFullPath((s).c_str()) ? std::string (s) : SEACAVE::Util::getSimplifiedPath((p)+(s))) // add the given path to the given file name
#define MAKE_PATH_REL(p,s)	((s).compare(0,(p).size(),p) ? std::string (s) : std::string (std::string ((s).substr((p).size())))) // remove the given path from the given file name
...
#define DEFVAR_OPTION(SPACE, flags, type, name, title, desc, ...) namespace SPACE { \
	...
	void update_##name() { \
		std::string ::FromString(oConfig[title].val, name); \
	} \
	void init_##name(SEACAVE::IDX idx) { \
		...
		std::string ::FromString(opt.defval, name); \
	} \
	LPCTSTR index_##name() { \
