...
/**************************************************************************************
 * std::stringHash template
 * ---------------
 * compile-time std::string hash template
 **************************************************************************************/
...
public:
	...
	static inline Key HashKey(const std::string& str) { return HashKey((const uint8_t*)str.c_str(), (Size)str.size()); }

	// Convenience functions
	inline		 Type*	Find		(LPCTSTR		key)			{ return  Find  ( HashKey(key) );			}
	inline		 Type*	Find		(const std::string&	key)			{ return  Find  ( HashKey(key) );			}
	inline const Type*	Find		(LPCTSTR		key)	const	{ return  Find  ( HashKey(key) );			}
	inline const Type*	Find		(const std::string&	key)	const	{ return  Find  ( HashKey(key) );			}
	inline void			Delete		(LPCTSTR		key)			{		  Delete( HashKey(key) );			}
	inline void			Delete		(const std::string&	key)			{		  Delete( HashKey(key.c_str()) );	}
	inline bool			Exists		(LPCTSTR		key)	const	{ return  Exists( HashKey(key) );			}
	inline bool			Exists		(const std::string&	key)	const	{ return  Exists( HashKey(key.c_str()) );	}
	inline		 Type&	operator [] (LPCTSTR		key)			{ return (*this)[ HashKey(key) ];			}
	inline		 Type&	operator [] (const std::string&	key)			{ return (*this)[ HashKey(key.c_str()) ];	}
	inline const Type&	operator [] (LPCTSTR		key)	const	{ return (*this)[ HashKey(key) ];			}
	inline const Type&	operator [] (const std::string&	key)	const	{ return (*this)[ HashKey(key.c_str()) ];	}
};
