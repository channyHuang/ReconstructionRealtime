...
public:
	static std::string getAppName() {
		...
		//return ensureUnifySlash(std::string(buf));
		return "";
		#else // _MSC_VER
		LPTSTR home = getenv("HOME");
		if (home == NULL)
			return std::string();
		std::string name(std::string(home) + "/app");
		return ensureUnifySlash(name);
		#endif // _MSC_VER
	}

	// generate a unique name based on process ID and time
	static std::string getUniqueName(TCHAR dash='-')
	{
		TCHAR szDate[256];
		#ifdef _MSC_VER
		SYSTEMTIME st;
		...
		//if (dash)
			//return std::string::Formatstd::string("%s%c%s%c%06X", szDate, dash, szTime, dash, ID);
		//return std::string::Formatstd::string("%s%s%06X", szDate, szTime, ID);
		return "";
	}
	...
	static std::string translateError(int aError) {
		...
		//std::string tmp((LPCTSTR)lpMsgBuf);
		//// Free the buffer.
		//LocalFree(lpMsgBuf);
		//std::string::size_type i;

		//while ((i = tmp.find_last_of(LINE_SEPARATOR_STR)) != std::string::npos)
	...
	static std::string& ensureUnifyReverseSlash(std::string& path)
	{
		std::string::size_type start = 0;
		while ((start = path.find(PATH_SEPARATOR, start)) != std::string::npos)
	...
	static std::string& ensureFolderSlash(std::string& path)
	{
		if (path.empty())
			return path;
		std::string::size_type nEnd = path.size()-1;
	...
	static std::string& ensureValidPath(std::string& path) {
		//return simplifyPath(ensureUnifySlash(strTrim(path, _T("\""))));
		return path;
	}
	static std::string& ensureValidFolderPath(std::string& path) {
	...
	static std::string getFullPath(const std::string& str) {
		//if (isFullPath(str))
		//	return str;
	...
	static std::string getProcessFolder() {
	...
	static std::string& simplifyPath(std::string& path) {
		...
		//while ((i = path.find(_T(".") PATH_SEPARATOR_STR, i)) != std::string::npos) {
		...
		//		std::string::size_type prev = path.rfind(PATH_SEPARATOR, i-2);
		//		if (prev == std::string::npos) prev = 0; else ++prev;
	...
	static std::string getSimplifiedPath(std::string path) {
		//return simplifyPath(path);
		return "";
	}

	static std::string getRelativePath(const std::string& currentPath, const std::string& targetPath) {
		...
		CLISTDEF2(std::string) currentPathValues, targetPathValues;
	...
	static std::string& getCommonPath(std::string& commonPath, const std::string& path) {
	...
	static std::string getCommonPath(const std::string* arrPaths, size_t numPaths) {
		// returns the path shared by all given paths
		ASSERT(numPaths > 0);
		std::string commonPath(arrPaths[0]);
	...
	static std::string getFilePath(const std::string& path) {
		const std::string::size_type i = path.rfind(PATH_SEPARATOR);
		return (i != std::string::npos) ? path.substr(0, i+1) : std::string();
	}
	static std::string getFileFullName(const std::string& path) {
		const std::string::size_type i = path.rfind('.');
		return (i != std::string::npos) ? std::string(path.substr(0, i)) : path;
	}
	...
	static std::string getFileName(const std::string& path) {
		std::string::size_type i = path.rfind(PATH_SEPARATOR);
		if (i == std::string::npos) i = 0; else ++i;
		std::string::size_type j = path.rfind('.');
		if (j == std::string::npos) j = path.length();
		return path.substr(i, j-i);
	}
	static std::string getFileExt(const std::string& path) {
		const std::string::size_type i = path.rfind('.');
		return (i != std::string::npos) ? path.substr(i) : std::string();
	}
	static std::string getLastDir(const std::string& path) {
		const std::string::size_type i = path.rfind(PATH_SEPARATOR);
		if (i == std::string::npos) return std::string();
		const std::string::size_type j = path.rfind(PATH_SEPARATOR, i-1);
		if (j != std::string::npos)
			return path.substr(j+1, j-i-1);
		return path;
	}
	static std::string insertBeforeFileExt(const std::string& path, const std::string& extra) {
		const std::string::size_type i = path.rfind('.');
		return (i != std::string::npos) ? std::string(path).insert(i, extra) : path+extra;
	}
	static std::string insertAfterFilePath(const std::string& path, const std::string& extra) {
		const std::string::size_type i = path.rfind(PATH_SEPARATOR);
		return (i != std::string::npos) ? std::string(path).insert(i+1, extra) : extra+path;
	}
	static int compareFileName(const std::string& path1, const std::string& path2) {
		//#ifdef _MSC_VER
	...
	static std::string getValue(const std::string& str, const std::string& label) {
		std::string::size_type pos = str.find(label);
		if (pos == std::string::npos)
			return std::string();
		pos = str.find('=', pos);
		if (pos == std::string::npos)
			return std::string();
		//std::string::size_type end = str.find_first_of(_T(LINE_SEPARATOR_STR), ++pos);
		//return str.substr(pos, end-pos);
		return "";
	}

	// remove all instances of a given sequence of characters from the beginning and end of the given string
	static std::string& strTrim(std::string& str, const std::string& strTrim) {
		if (str.empty())
			return str;
	...
	static void strSplit(const std::string& str, TCHAR delim, CLISTDEF2(std::string)& values) {
		values.Empty();
	...
	static void strSplit(const std::string& str, const std::string& delim, CLISTDEF2(std::string)& values) {
		ASSERT(!delim.empty());
		values.Empty();
		std::string::size_type start(0), end(0);
		while (end != std::string::npos) {
	...
	static std::string getTimString() {
		char buf[64];
		time_t _tt;
		time(&_tt);
		tm* _tm = localtime(&_tt);
	...
	static std::string formatBytes(int64_t aBytes) {
		...
		//	return std::string::Formatstd::string("%dB", (uint32_t)aBytes&0xffffffff);
		//} else if (aBytes < (int64_t)1024*1024) {
		//	return std::string::Formatstd::string("%.02fKB", (double)aBytes/(1024.0));
		//} else if (aBytes < (int64_t)1024*1024*1024) {
		//	return std::string::Formatstd::string("%.02fMB", (double)aBytes/(1024.0*1024.0));
		//} else if (aBytes < (int64_t)1024*1024*1024*1024) {
		//	return std::string::Formatstd::string("%.02fGB", (double)aBytes/(1024.0*1024.0*1024.0));
		//} else {
		//	return std::string::Formatstd::string("%.02fTB", (double)aBytes/(1024.0*1024.0*1024.0*1024.0));
		//}
		return "";
	}

	// format given time in milliseconds to higher units
	static std::string formatTime(int64_t sTime, uint32_t nAproximate = 0) {
		char buf[128];
		uint32_t len = 0;
	...
	static std::string toString(const wchar_t* wsz) {
		if (wsz == NULL)
			return std::string();
		#if 1
	...
	static std::string CommandLineToString(size_t argc, LPCTSTR* argv) {
		std::string strCmdLine;
		//for (size_t i=1; i<argc; ++i)
		//	strCmdLine += _T(" ") + std::string(argv[i]);
		return strCmdLine;
	}

	struct Progress {
		const std::string msg; // custom message header (ex: "Processed images")
		...
		Progress(const std::string& _msg, size_t _total, Timer::Type _slp=100/*ms*/)
		...
		void process() {
			...
			// display progress
			//print(std::string::Formatstd::string(_T("%s %u (%.2f%%, %s, ETA %s)..."), msg.c_str(), done, percentage*100.f, formatTime((int64_t)elapsed,1).c_str(), formatTime((int64_t)remaining,2).c_str()));
		}
		void close() {
			// make sure we print the complete progress
			const Timer::Type elapsed(Timer::SysTime2TimeMs(Timer::GetSysTime()-start));
			// display progress
			//print(std::string::Formatstd::string(_T("%s %u (100%%, %s)"), msg.c_str(), total, formatTime((int64_t)elapsed).c_str()));
			std::cout << _T("\n");
			processed = 0;
		}
		void print(const std::string& line) {
			...
			if (lastMsgLen > msgLen)
				std::cout << std::string(lastMsgLen-msgLen, _T(' '));
			std::cout << std::flush;
			lastMsgLen = msgLen;
		}
	};
