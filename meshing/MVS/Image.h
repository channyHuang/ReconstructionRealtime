...
public:
	...
	std::string name; // image file name (relative path)
...
public:
	...
	// static IMAGEPTR OpenImage(const std::string& fileName);
	// static IMAGEPTR ReadImageHeader(const std::string& fileName);
	// static IMAGEPTR ReadImage(const std::string& fileName, Image8U3& image);
	// static bool ReadImage(IMAGEPTR pImage, Image8U3& image);
	bool LoadImage(const std::string& fileName, unsigned nMaxResolution=0);
	...
	void save(Archive& ar, const unsigned int /*version*/) const {
		...
		ar & poseID;
		ar & ID;
		const std::string relName(MAKE_PATH_REL(WORKING_FOLDER_FULL, name));
		ar & relName;
		ar & width & height;
		ar & neighbors;
		ar & avgDepth;
