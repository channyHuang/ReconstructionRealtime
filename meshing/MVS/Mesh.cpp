...
#define DEFINE_REMOVE4( go2 )                                                                                                                                                                                  \
  ...
  DEFINE_FACES4( 0, 1, 2, 3, go2 )                                                                                                                                                                             \
  else DEFINE_FACES4( 0, 1, 3, 2, go2 ) else DEFINE_FACES4( 0, 2, 1, 3, go2 ) else DEFINE_FACES4( 0, 2, 3, 1, go2 ) else DEFINE_FACES4( 0, 3, 1, 2, go2 ) else DEFINE_FACES4( 0, 3, 2, 1, go2 ) else goto go2; \
...
bool Mesh::FixNonManifold()
{
  ...
  // printf( "Fixed %u/%u non-manifold edges/vertices and %u faces removed: %u pyramid3 and %u pyramid4 (%s)\r\n", nNonManifoldEdges, nNonManifoldVertices, nRemoveFaces, nPyramid3, nPyramid4, TD_TIMER_GET_FMT().c_str() );
...
{
  TD_TIMER_STARTD();
  const std::string ext( Util::getFileExt( fileName ) );
  bool         ret;
  if ( ext == (".obj") )
...
//    }
//    else if ( PLY::equal_strings( BasicPLY::elem_names[ 1 ], elem_name ) )
//    {
...
//        {
//          if ( std::strncmp( comment.c_str(), ("TextureFile "), 12 ) == 0 )
//          {
//            const std::string textureFileName( comment.substr( 12 ) );
...
bool Mesh::Save( const std::string &fileName, const cList< std::string > &comments, bool bBinary ) const
{
  TD_TIMER_STARTD();
  const std::string ext( Util::getFileExt( fileName ) );
  std::cout << "Save mesh to file: " << fileName << std::endl;
  bool         ret;
  if ( ext == (".obj") )
    ret = SaveOBJ( fileName );
  //else
  //  ret = SavePLY( ext != (".ply") ? std::string( fileName + (".ply") ) : fileName, comments, bBinary );
  if ( !ret )
    return false;
  // DEBUG_EXTRA( "Mesh saved: %u vertices, %u faces (%s)", vertices.GetSize(), faces.GetSize(), TD_TIMER_GET_FMT().c_str() );
  printf( "Mesh saved: %u vertices, %u faces (%s)\r\n", vertices.GetSize(), faces.GetSize(), TD_TIMER_GET_FMT().c_str() );
  return true;
}

// export the mesh as a PLY file
//bool Mesh::SavePLY( const std::string &fileName, const cList< std::string > &comments, bool bBinary ) const
//{
//  ASSERT( !fileName.IsEmpty() );
...
bool Mesh::Save( const VertexArr &vertices, const std::string &fileName, bool bBinary )
{
  ASSERT( !fileName.empty() );
  Util::ensureFolder( fileName );
...
public:
  ...
  inline Halfedge_const_handle get_edge( int index ) const
  {
    ...
    case 2:
      return this->halfedge()->next()->next();
    }
  ...
  inline double edgeStatistics( int mode = 1 ) const
  { // 0 - min; 1-avg; 2-max
    ASSERT( isTrinagle() );
    ...
    {
    case 0:
      return MINF3( e1, e2, e3 );
    case 1:
      return ( e1 + e2 + e3 ) / 3;
    case 2:
      return MAXF3( e1, e2, e3 );
    }
...
static bool CanFlipEdge( Vertex::Halfedge_handle h )
{
  ...
  if ( ( h->next() == null_h ) || ( h->prev() == null_h ) || ( h->opposite() == null_h ) || ( h->opposite()->next() == null_h ) )
    return false;
  ...
  if ( v0->flags.isSet( Vertex::FLG_EULER ) )
    return false;
  ...
  if ( ( a1 * a1 + b1 * b1 ) / ( a2 * a2 + a3 * a3 + b2 * b2 + b3 * b3 ) < 1.01 )
    return false;
  ...
  if ( v_angle( v_perp_1, v_perp_2 ) > D2R( 20 ) )
    return false;
...
static float ComputeVertexStatistics( Vertex &v, int mode )
{
  ...
  do
  {
    ...
    {
    case 0:
      edge_stats = MINF( ( float ) edge_size( h ), edge_stats );
      break;
    case 1:
      edge_stats += ( float ) edge_size( h ), ++no_h;
      break;
    case 2:
      edge_stats = MAXF( ( float ) edge_size( h ), edge_stats );
      break;
    }
  } while ( ++h != v.vertex_begin() );
  if ( mode == 1 )
    edge_stats = ( edge_stats / no_h );
  return edge_stats;
}
  ...
  for ( Facet_iterator fi = p.facets_begin(); fi != p.facets_end(); )
  {
    ...
    for ( i = 0; i < 3; i++ )
    {
      if ( ( edge[ i ] / edge[ ( i + 1 ) % 3 ] < collapseRatio ) && ( edge[ i ] / edge[ ( i + 2 ) % 3 ] < collapseRatio ) )
      {
        ...
        {
          while ( ( fi != p.facets_end() ) && ( fi == edge_h[ i ]->opposite()->facet() ) )
            fi++;
  ...
  for ( Facet_iterator i = p.facets_begin(); i != p.facets_end(); ++i )
  {
    ...
    { // start a new component
      ...
      { // fill the current component
        Vertex::Facet_handle f = facet_queue.front();
        facet_queue.pop();
        lastComponent.size++;
...
static void Smooth( Polyhedron &p, double delta, int mode = 0 )
{
  ...
  for ( Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end(); vi++ )
  {
    ...
    break;
    case 6:
      displacement = vi->laplacian * delta - vi->normal * laplacian.avg * delta;
      break;
    }
...
static void ComputeVertexNormals( Polyhedron &p )
{
  ...
  for ( Facet_iterator fi = p.facets_begin(); fi != p.facets_end(); fi++ )
  {
    ...
    Vector &     n0( fi->get_vertex( 0 )->normal );
    n0 = n0 + t;
    Vector &n1( fi->get_vertex( 1 )->normal );
    n1 = n1 + t;
    Vector &n2( fi->get_vertex( 2 )->normal );
    n2 = n2 + t;
  }
  for ( Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end(); vi++ )
  {
...
static std::vector< std::pair< Vertex *, int > > GetRingNeighbourhood( Vertex &v, int ring_size, bool include_original )
{
  ...
  while ( elems.size() > 0 )
  {
    ...
    CGAL_For_all( c, d )
    {
      ...
      if ( iter == neigh_map.end() )
      { // if the vertex has not been already taken
        elems.push( next_el );
...
// add the facets
    ...
    FOREACH( i, faces )
    {
      ...
      if ( !B.test_facet( f.ptr(), f.ptr() + 3 ) )
      {
        ...
        //  msgFaces += std::string::Formatstd::string( " %u", i );
#endif
        continue;
      }
...
void Mesh::Subdivide( const AreaArr &maxAreas, uint32_t maxArea )
{
  ...
  FacetSplitMap mapSplits;
  mapSplits.reserve( faces.GetSize() );
  FacetCountMap mapFaces;
  mapFaces.reserve( 12 * 3 );
