...
bool ObjModel::MaterialLib::Save(const std::string& prefix, bool texLossless) const
{
	...
	const std::string pathName(Util::getFilePath(prefix));
	const std::string name(Util::getFileNameExt(prefix));
	...
	for (int_t i = 0; i < (int_t)materials.size(); ++i) {
		...
		if (mat.diffuse_name.empty())
			const_cast<std::string&>(mat.diffuse_name) = name+"_"+mat.name+"_map_Kd."+(texLossless?"png":"jpg");
...
bool ObjModel::Save(const std::string& fileName, unsigned precision, bool texLossless) const
{
	if (vertices.empty())
		return false;
	const std::string prefix(Util::getFileFullName(fileName));
	const std::string name(Util::getFileNameExt(prefix));
...
bool ObjModel::Load(const std::string& fileName)
{
	...
	while (fin.good()) {
		...
		if (keyword == "f") {
			...
			for (size_t k = 0; k < 3; ++k) {
				in >> keyword;
				switch (sscanf(keyword.c_str(), "%u/%u/%u", f.vertices+k, f.texcoords+k, f.normals+k)) {
				case 1:
